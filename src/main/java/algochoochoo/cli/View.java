...
public class View {
    ...
    while (true) {
      ...
      if (command.getExecutable().equals("search")) {
        ...
        case "priority":
          ...
          default:
            System.err.println(
                "Error: unknown priority: " + command.getArguments()[2] +
                " available: (time, trips)");
            break;
          }
          break;
        case "gtfs-path":
        ...
        case "foot-radius":
          ...
          try {
            graphset.foot_radius =
                (int)Math.round(Double.parseDouble(command.getArguments()[2]));
            System.err.println("Reloading graph...");
            graph.reload(graphset);
          } catch (Exception e) {
            System.err.println("Error: unable to reload graph - " +
        ...
        case "train-weight":
          ...
          try {
            ...
            System.err.println("Error: cannot apply weight - " +
                               e.getMessage() + "\n");
            continue;
          }
          if (w <= 0.0 || w > 10.0) {
            System.err.println(
                "Error: transport weight is not in the range (0, 10]\n");
            continue;
          }
          RouteType type =
              command.getArguments()[0].equals("foot-weight") ? RouteType.FOOT
              : command.getArguments()[0].equals("metro-weight")
                  ? RouteType.METRO
              : command.getArguments()[0].equals("tram-weight") ? RouteType.TRAM
              : command.getArguments()[0].equals("bus-weight")
                  ? RouteType.BUS
                  : RouteType.TRAIN;
        ...
        default:
          System.err.println(
              "Error: unknown setting: " + command.getArguments()[0] +
              ", see help to seek help");
          break;
        }
        ...
        case "gtfs-path":
          ...
          break;
        case "foot-radius":
          System.out.println("Footpath radius = " + graphset.foot_radius +
                             " meters");
          break;
        case "weights":
          System.out.println(
              "weights = [foot: " +
              algoset.weights.getOrDefault(RouteType.FOOT, 1.0) + ", metro: " +
              algoset.weights.getOrDefault(RouteType.METRO, 1.0) +
        ...
        case "train-weight":
          RouteType type =
              ...
              : command.getArguments()[0].equals("bus-weight")
                  ? RouteType.BUS
                  : RouteType.TRAIN;
          System.out.println(command.getArguments()[0] + " = " +
                             algoset.weights.getOrDefault(type, 1.0));
          break;
        default:
          System.err.println(
              "Error: unknown setting: " + command.getArguments()[0] +
        ...
        System.out.println(
            "Algochoochoo Env v1.0 - Bilal Vandenberge\n\nCommands:\n");
        System.out.println(" search <s> <t> <h>\t\tSearch the minimum cost "
                           + "path from s to t at h");
        System.out.println(
            " update <setting> = <value>\tUpdate a settings with a new value");
        System.out.println(" get <setting>\t\t\tGet a settings value");
        System.out.println(" help\t\t\t\tPrint help page");
        System.out.println(
            "\nSettings:\n name\t\tdescription\t\t\t\t\t\t\tvalues\n");
        System.out.println(
            " gtfs-path\tThe path of the GTFS directory\t\t\t\t\tstring");
        System.out.println(" foot-radius\tThe maximum distance of a "
                           + "footpath\t\t\t\tpositive integer");
        System.out.println(" priority\tThe criterion that is minimized by "
                           + "the search algorithm\t\ttime OR trips");
        System.out.println(" weights\tThe weights for each transport "
                           + "type\t\t\t\tmap[foot, metro, tram, bus, train]");
      } else {
        System.err.println("Error: unknown command: " +
                           command.getExecutable() + ", see help to seek help");
      }
      System.out.println();
    }
    scanner.close();
  ...
  public static void print(AlgoResult result, int h) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      System.out.print(" from " + current.from().stop().name() +
                       beautiful_time(time));
      ...
      i = j - 1;
      System.out.println(" to " + path.get(i).to().stop().name() +
                         beautiful_time(time));
    }
    System.out.println();
  ...
  private static final List<String> EXIT_KEYWORDS =
      List.of("exit", "quit", "q");
