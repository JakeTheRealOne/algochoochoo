 ...
 * associated with each key—the client uses this integer to specify which key
 * to delete or change. It also supports methods for peeking at the minimum
...
public class IndexMinPQ<Key extends Comparable<Key>>
  ...
  public void decreaseKey(int i, Key key) {
    ...
    if (keys[i].compareTo(key) == 0)
      throw new IllegalArgumentException(
          "Calling decreaseKey() with a key equal to the key in the priority "
          + "queue");
    if (keys[i].compareTo(key) < 0)
      throw new IllegalArgumentException(
          "Calling decreaseKey() with a key strictly greater than the key in "
          + "the priority queue");
    keys[i] = key;
    swim(qp[i]);
  }
  ...
  public void increaseKey(int i, Key key) {
    ...
    if (keys[i].compareTo(key) == 0)
      throw new IllegalArgumentException(
          "Calling increaseKey() with a key equal to the key in the priority "
          + "queue");
    if (keys[i].compareTo(key) > 0)
      throw new IllegalArgumentException(
          "Calling increaseKey() with a key strictly less than the key in the "
          + "priority queue");
    keys[i] = key;
    sink(qp[i]);
  }
