...
/*
 *  The IndexFibonacciMinPQ class represents an indexed priority queue of
 * generic keys. It supports the usual insert and delete-the-minimum
 * operations, along with delete and change-the-key methods. In order to let
 * the client refer to keys on the priority queue, an integer between 0 and N-1
 * is associated with each key ; the client uses this integer to specify which
 * key to delete or change. It also supports methods for peeking at the minimum
...
public class IndexFibonacciMinPQ<Key> implements Iterable<Integer> {
   ...
   * Worst case is O(1)
   * @param i an index
   * @throws java.lang.IllegalArgumentException if the specified index is
   *     invalid
